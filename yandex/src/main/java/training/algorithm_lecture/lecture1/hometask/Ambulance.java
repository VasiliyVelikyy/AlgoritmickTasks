...
public class Ambulance {
    ...
    private static void process() {
        try (Scanner scanner = new Scanner(System.in)) {
            int K1 = scanner.nextInt(); // Номер нужной кв 11
            int M = scanner.nextInt(); // Этажей 1
            int K2 = scanner.nextInt(); // Недавняя кв 1
            int P2 = scanner.nextInt(); // Недавний подъезд 1
            int N2 = scanner.nextInt(); // Недавний этаж 1
            ...
            if (K2 >= (P2 - 1) * M + N2 && (P2 - 1) * M + N2 > 1) {
                ...
                int N1 = (int) Math.ceil((double) K1 / K_Emin);
                int P1 = (int) Math.ceil((double) N1 / M);
                N1 = (N1 - 1) % M + 1;
